...
class Many2Many_multiselect
{
    ...
    public function input_form($input_array, $cell_id)
    {
        ...
        <?php foreach ($res as $join_row) { ?>
             <option value="<?= $join_row[$input_array['rel_join']] ?>" selected><?= $join_row[$input_array['join_name']] ?></option>
        <?php } ?>
        </select>
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            for (var index in data) {
                                $('.multiselect_<?= $input_array['name'] ?>').append('<option value="' + data[index].<?=$input_array['join_key']?> + '">' + data[index].<?=$input_array['join_name']?> +'</option>');
                            }
